...
class LessParser extends \XLite\Base\Singleton
{
    ...
    protected function getCacheDir($media)
    {
        return LC_DIR_CACHE_RESOURCES
            . (\XLite::isAdminZone() ? 'admin' : 'default') . LC_DS
            . (\XLite\Core\Request::getInstance()->isHTTPS() ? 'https' : 'http') . LC_DS
            . $media . LC_DS;
    }
    ...
    {
        $filePrefix = str_replace(LC_DS, '/', dirname(substr($data['file'], strlen(LC_DIR_ROOT)))) . '/';
