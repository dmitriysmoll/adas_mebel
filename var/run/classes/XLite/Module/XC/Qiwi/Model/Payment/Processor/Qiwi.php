...
class Qiwi extends \XLite\Model\Payment\Base\WebBased
{
    ...
    {
        return parent::isConfigured($method) && $method->getSetting('login') && $method->getSetting('password');
    }
    ...
    protected function createInvoice()
    {
        ...
        if (0 != $response->response->result_code) {
            $billId = 0;
        }

        Database::getEM()->flush();

        return array($billId, $response->response->status);
    }
