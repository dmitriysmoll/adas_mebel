...
function checkFilePermissions(&$errorMsg, &$value)
{
    ...
    if (count($perms) > 0) {
        $result = false;
        if (LC_OS_IS_WIN) {
            $errorMsg = xtr("Permissions checking failed. Please make sure that the following files have writable permissions:<br /><br /><i>:perms</i>", array(':perms' => implode("<br />", $perms)));

        } else {
            $errorMsg = xtr("Permissions checking failed. Please make sure that the following file permissions are assigned (UNIX only):<br /><br /><i>:perms</i>", array(':perms' => implode("<br />", $perms)));
        }
    }
...
function displayFormElement($fieldName, $fieldData, $clrNumber)
{
    ...
    switch ($fieldType) {
        ...
        case 'select': {
            ...
            $formElement =<<<OUT
        <select class="form-control" name="params[{$fieldName}]">
OUT;
...
OUT;
        ...
        default: {
            ...
            $formElement =<<<OUT
        <input class="form-control" type="{$fieldType}" name="params[{$fieldName}]" class="full-width" value="{$fieldValue}" />
OUT;
        }
    }
...
function module_install_cache(&$params)
{
    ...
    if ('1' == $params['new_installation'] && 'Y' == $params['demo']) {
        ...
        if (file_exists($dump_file) && is_readable($dump_file)) {
            ...
            $sql = str_replace('`FK_', '`FK_' . rand(0, 99), $sql);
            $connection = dbConnect($params, $pdoErrorMsg);
