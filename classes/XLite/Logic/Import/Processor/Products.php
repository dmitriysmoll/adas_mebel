...
class Products extends \XLite\Logic\Import\Processor\AProcessor
{
    ...
    protected function isProductExists()
    {
        ...
        if (!\XLite\Core\Converter::isEmptyString($this->currentRowData['sku'])) {
            $result = \XLite\Core\Database::getRepo('XLite\Model\Product')->findOneBy(array('sku' => $this->currentRowData['sku']));
        }
    ...
    /**
     * Normalize 'inventoryTrackingEnabled' value
     *
     * @param mixed @value Value
     *
     * @return boolean
     */
    protected function normalizeInventoryTrackingEnabledValue($value)
    {
        return $this->normalizeValueAsBoolean($value);
    }

    /**
     * Normalize 'lowLimitEnabled' value
     *
     * @param mixed @value Value
     *
     * @return boolean
     */
    protected function normalizeLowLimitEnabledValue($value)
    {
        return $this->normalizeValueAsBoolean($value);
    }
