...
class Review extends \XLite\Controller\Admin\AAdmin
{
    ...
    protected function doActionCreate()
    {
        ...
        if (intval($data['profile_id']) > 0) {
            $profile = \XLite\Core\Database::getRepo('XLite\Model\Profile')->find($data['profile_id']);
        ...
        $product = $this->getRequestProduct() ? : $this->findProductBySubstring($data['product_title']);
