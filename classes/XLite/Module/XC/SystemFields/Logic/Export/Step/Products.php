...
abstract class Products extends \XLite\Logic\Export\Step\Products implements \XLite\Base\IDecorator
{
     ...
     * @return string
     */
    protected function getUpcIsbnColumnValue(array $dataset, $name, $i)
    {
        return $this->getColumnValueByName($dataset['model'], 'upcIsbn');
    }

    /**
     * Get column value for 'mnfVendor' column
     *
     * @param array   $dataset Dataset
     * @param string  $name    Column name
     * @param integer $i       Subcolumn index
     ...
     * @return string
     */
    protected function getMnfVendorColumnValue(array $dataset, $name, $i)
    {
        return $this->getColumnValueByName($dataset['model'], 'mnfVendor');
    }
}
