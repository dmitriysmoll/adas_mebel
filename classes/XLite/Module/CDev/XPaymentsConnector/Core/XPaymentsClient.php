...
class XPaymentsClient extends \XLite\Base\Singleton
{
     ...
     * @return array
     */
    public function getApiRequest($target, $action, array $data = array(), $schema = '', $apiVersion = false)
    {
        $result = null;
        ...
        if (!$apiVersion) {
            // Substitute API version if necessary
            $apiVersion = $config->xpc_api_version;
        }
    ...
    protected function requestPaymentInit(\XLite\Model\Payment\Method $paymentMethod, \XLite\Model\Cart $cart, $forceAuth = false)
    {
        ...
        if (!$cart || !$preparedCart) {
            ...
            if ('1.3' == \XLite\Core\Config::getInstance()->CDev->XPaymentsConnector->xpc_api_version) {
                ...
                $data13['saveCard'] = 'Y' == \XLite\Core\Request::getInstance()->save_card
                    ? 'Y'
                    : 'N';
 ...
 <xsd:simpleType>
  <xsd:restriction base="xsd:string">
   ...
   <xsd:minLength value="32"/>
     ...
     * @return string
     */
    public function getRequestInitSchema()
    {
        return '
 ...
 <xsd:complexType>
  <xsd:sequence>
   ...
   <xsd:element name="settingsHash" type="xsd:string"/>';
    ...
    <xsd:element name="canSaveCards" type="xsd:string"/>'; 
        }
