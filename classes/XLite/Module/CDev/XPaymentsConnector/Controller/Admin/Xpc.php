...
class Xpc extends \XLite\Controller\Admin\Module
{
    ...
    {
        $result = \XLite\Module\CDev\XPaymentsConnector\Core\Settings::getInstance()->importPaymentMethods();

        if (empty($result)) {
            \XLite\Core\TopMessage::getInstance()->addInfo('Payment methods have been successfully imported');

        } elseif ('E' == $result['type']) {
            \XLite\Core\TopMessage::addError($result['message']);

        } else {
            \XLite\Core\TopMessage::addWarning($result['message']);
        }
    }
