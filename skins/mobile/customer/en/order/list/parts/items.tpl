...
<table IF="getPageData()" data-role="table" data-mode="reflow" class="ui-responsive table-stroke order-list">
  ...
  <tbody>
    <tr FOREACH="getPageData(),order">
      ...
      <td class="payment-status"><widget class="\XLite\View\OrderStatus\Payment" order="{order}" useWrapper="true" /></td>
      <td class="shipping-status"><widget class="\XLite\View\OrderStatus\Shipping" order="{order}" useWrapper="true" /></td>
      <td class="buttons">
        <div>
          <widget
